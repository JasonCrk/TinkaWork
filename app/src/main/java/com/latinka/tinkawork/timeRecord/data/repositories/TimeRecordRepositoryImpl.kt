...
class TimeRecordRepositoryImpl : TimeRecordRepository {
    ...
    override fun findEntryTimeByUser(userRef: DocumentReference): Task<QuerySnapshot> {
        return db.collection(FireStoreCollections.TIME_RECORDS)
            .whereEqualTo(TimeRecordFields.USER, userRef)
            .whereEqualTo(TimeRecordFields.IS_ENTRY, true)
            .orderBy(TimeRecordFields.CREATED_AT, Query.Direction.DESCENDING)
            .get()
    }

    override fun getDepartureByEntryTimeAndUser(
        entryTime: Date,
        userRef: DocumentReference
    ): Task<QuerySnapshot> {
        val peruTimezone = TimeZone.getTimeZone("America/Lima")

        val entryTimeCalendar = Calendar.getInstance(peruTimezone)
        entryTimeCalendar.time = entryTime

        val endDay = Calendar.getInstance(peruTimezone).apply {
            set(Calendar.DAY_OF_MONTH, entryTimeCalendar.get(Calendar.DAY_OF_MONTH))
            set(Calendar.HOUR_OF_DAY, 23)
            set(Calendar.MINUTE, 59)
            set(Calendar.SECOND, 59)
            set(Calendar.MILLISECOND, 999)
        ...
        return db.collection(FireStoreCollections.TIME_RECORDS)
            .whereEqualTo(TimeRecordFields.USER, userRef)
            .whereEqualTo(TimeRecordFields.IS_ENTRY, false)
            .whereGreaterThanOrEqualTo(TimeRecordFields.CREATED_AT, Timestamp(entryTime))
            .whereLessThanOrEqualTo(TimeRecordFields.CREATED_AT, Timestamp(endDay.time))
            .get()
    }
